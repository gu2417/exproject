...
본 시스템은 콘솔 클라이언트와 TCP 서버가 요청/응답 패킷을 주고받는 구조로 설계한다.  
클라이언트는 사용자 입력을 서버가 이해할 수 있는 요청 문자열로 변환하고, 서버는 CSV에서 로드한 교통 데이터를 기반으로 검색, 길찾기, 편의시설, 사용자 기록 기능을 처리한다.  
아래 도식은 전체 구조, 기능 모듈, 요청 처리 흐름, 데이터 로딩 과정을 텍스트 구조도로 정리한 것이다.
...
|  +----------------------------------------------------+  |
|  | Service Layer                                      |  |
|  | stop search | bus search | subway search           |  |
|  | transfer path | facility | favorite | recent        |  |
|  | arrival time | crowding status                     |  |
|  +-------------------------+--------------------------+  |
|                            |                             |
|  +-------------------------v--------------------------+  |
|  | In-Memory Data Store                                |  |
|  | linked lists: stops, routes, stations, facilities   |  |
|  | graph: bus/subway/transfer network                 |  |
|  +-------------------------+--------------------------+  |
|                            |                             |
|  +-------------------------v--------------------------+  |
|  | CSV Loader / File Writer                            |  |
|  | data/*.csv | favorites.txt | recent_search.txt      |  |
|  | logs/server.log                                    |  |
|  +----------------------------------------------------+  |
+----------------------------------------------------------+
                  ^ TCP Socket ^
...
|           |                                               |
|           v                                               |
|  menu input | formatted result output                    |
+----------------------------------------------------------+
```

클라이언트 영역은 메뉴 입력, 서버 통신, 결과 출력 기능으로 구성된다.  
서버 영역은 접속 대기, 요청 수신, 라우팅, 서비스 처리 기능으로 구성되며, 데이터 영역의 CSV 파일과 사용자 저장 파일을 사용한다.  
공통 모듈은 클라이언트와 서버가 함께 사용하는 통신 규칙, 자료형, 문자열 처리, CSV 파싱, 그래프 탐색 기능을 제공한다.

### 2-2. 주요 기능 모듈 구성도

```text
+-----------------------+        +-----------------------+
|      Search Menu      |        |    Transfer Menu      |
|  stop / bus / subway  |        |  start -> destination |
+-----------+-----------+        +-----------+-----------+
            |                                |
            v                                v
+-----------+-----------+        +-----------+-----------+
|  search_service.c     |        | transfer_service.c    |
|  - stop search        |        |  - node matching      |
|  - bus route search   |        |  - shortest path      |
|  - subway search      |        |  - transfer count     |
+-----------+-----------+        +-----------+-----------+
            |                                |
            +------------+      +------------+
                         |      |
        ...
        +----------------+------+----------------+
        |             Common Modules             |
        | protocol.h | types.h | csv_parser      |
        | linked_list | graph | string_utils     |
        +----------------+------+----------------+
                         ^      ^
            +------------+      +------------+
            |                                |
+-----------+-----------+        +-----------+-----------+
| facility_service.c    |        | user_state_service.c  |
| - nearby facilities   |        | - favorite add/list   |
| - category filtering  |        | - favorite delete     |
| - nearest node match  |        | - recent search list  |
+-----------+-----------+        +-----------+-----------+
            ^                                ^
            |                                |
+-----------+-----------+        +-----------+-----------+
|    Facility Menu      |        |  Favorite / Recent    |
|  place + category     |        |  saved user records   |
+-----------------------+        +-----------------------+
```

주요 기능은 검색, 길찾기, 편의시설 조회, 사용자 기록 관리로 나눌 수 있다.  
각 기능은 서버의 전담 모듈에서 처리하며, 공통 처리 기반으로 `protocol.h`, `types.h`, `csv_parser`, `graph`, `linked_list`를 사용한다.

### 2-3. 요청·응답 처리 흐름도

```text
+-------------+     +----------------+     +----------------+
| User Input  | --> | Console Menu    | --> | Request String |
| menu number |     | menu.c          |     | PKT_TYPE|data  |
+-------------+     +----------------+     +-------+--------+
                                                     |
                                                     v
                                             +-------+--------+
                                             ...
                                             +-------+--------+
                                              ...
                                              TCP request
                                                     ...
                                                     v
+-------------+     +----------------+     +--------+-------+
| CSV / Files | <-- | Service Module | <-- | Command Router |
| data store  |     | search/route   |     | router.c       |
+------+------+     | favorite/etc.  |     +--------+-------+
       |            +--------+-------+              ^
       |                     |                      |
       v                     v                      |
+------+------+     +--------+-------+              |
| Linked List |     | Response String |--------------+
| Graph       |     | PKT_RES|code|data              |
+-------------+     +--------+-------+
                             |
                         TCP response
                             |
...
사용자가 메뉴에서 기능을 선택하면 클라이언트가 요청 문자열을 생성하고 TCP 소켓으로 서버에 전달한다.  
서버는 요청 종류를 확인한 뒤 알맞은 서비스 함수로 분기하고, 처리 결과를 응답 문자열로 만들어 클라이언트에 반환한다.  
클라이언트는 응답 종류와 상태 코드를 파싱하여 검색 결과, 경로, 메시지 등을 화면에 출력한다.

### 2-4. 데이터 로딩 및 그래프 생성도
...
| StationNode list        |
| FacilityNode list       |
| Arrival/Crowding list   |
+------------+------------+
             |
             v
+------------+------------+
|    Transfer Graph       |
| GraphNode: stop/station |
| GraphEdge: bus route    |
| GraphEdge: subway line  |
| GraphEdge: transfer     |
+------------+------------+
             |
             v
...
| shortest path           |
| nearby facility         |
| arrival/crowding info   |
+-------------------------+
```

서버는 프로그램 시작 시 정류장, 노선, 지하철역, 편의시설, 도착 예정 시간, 혼잡도 CSV를 순서대로 읽는다.  
읽은 데이터는 연결리스트 기반 메모리 구조에 저장하고, 정류장과 지하철역은 그래프 노드로 등록한다.  
버스 노선, 지하철 역간 거리, 정류장-역 환승 가능 구간은 그래프 간선으로 연결하여 길찾기 기능에서 사용한다.
...
| FR-S04 | 길찾기 최단 경로 | 탑승 위치와 도착 위치를 입력하면 그래프 탐색으로 최단 경로를 제공한다 |
...
| FR-U04 | 즐겨찾기 삭제 | 저장된 즐겨찾기를 목록 번호로 삭제한다 |
| FR-U05 | 최근 검색 기록 저장 | 최근 검색한 키워드와 검색 유형을 `recent_search.txt`에 저장한다 |
| FR-U06 | 최근 검색 기록 조회 | 이전 검색 내용을 확인할 수 있는 메뉴 UI를 제공한다 |
| FR-U07 | 메뉴 기반 UI | 최종 메뉴 구성에 맞춘 콘솔 인터페이스를 제공한다 |
...
| 길찾기 | 탑승 위치, 도착 위치 | 최단 경로, 환승 횟수, 예상 소요시간 |
| 주변 편의시설 조회 | 정류장/역명, 카테고리 | 편의시설 이름, 종류, 주소 |
| 즐겨찾기 조회 | 없음 | 저장된 즐겨찾기 목록 |
| 즐겨찾기 추가 | 유형, ID 또는 번호, 표시 이름 | 저장 성공/중복 안내 |
| 즐겨찾기 삭제 | 삭제할 목록 번호 | 삭제 성공/실패 안내 |
...
| P3 (편의) | FR-U01~U07, FR-N04 | 즐겨찾기, 최근 검색, 로그 저장으로 사용성 및 검증 가능성 확보 |
...
| 추가 자료구조 적용 | FR-DATA03, FR-S04, NFR-05 | 길찾기 최단 경로에 그래프 자료구조 사용 |
| 공공데이터 CSV 사용 | FR-DATA01, NFR-04 | 공공데이터포털 CSV를 로드하고 검색 기능에 활용 |
| 파일 입출력 | FR-DATA01, FR-U04, FR-U05, FR-N04 | CSV 읽기, 즐겨찾기 삭제, 최근 검색 저장, 서버 로그 저장 동작 |
| 정류장 검색 | FR-S01 | 정류장별 경유 버스 조회 가능 |
| 버스 검색 | FR-S02 | 버스 번호별 경유 정류장 순서 조회 가능 |
| 지하철역 검색 | FR-S03 | 역명/키워드 검색 가능 |
| 즐겨찾기 | FR-U01~U04 | 추가, 조회, 삭제, 중복 방지 가능 |
| 최근 검색 기록 | FR-U05~U06 | 최근 검색 내용이 txt 파일에 저장되고 메뉴에서 조회 가능 |
...
| 최종 메뉴 구성 | FR-U07 | overview.md의 1~9, 0번 메뉴와 일치 |
